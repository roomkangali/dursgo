...
- `cmdinjection` - Detects Command Injection vulnerabilities (supports OAST).
...
### 1. Quick Scan for XSS and SQLi
```bash
./dursgo -u http://example.com/search?q=test -s xss,sqli -v
```

### 2. Authenticated Scan
```bash
./dursgo -u http://example.com/dashboard -s idor -config config-auth.yaml
```

### 3. OAST Scan for Blind SSRF
```bash
./dursgo -u http://example.com/export?url= -s blindssrf -oast
```
